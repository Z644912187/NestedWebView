...
public class NWebView extends WebView implements NestedScrollingChild {
    ...
    private static String TAG = NWebView.class.getSimpleName();
    ...
    public NWebView(Context paramContext) {
        this(paramContext, null);
    }

    public NWebView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NWebView(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
...
//                    setJavaScriptEnable(true);
    ...
    public static class MyWebViewClient extends WebViewClient {
        ...
        public final boolean shouldOverrideUrlLoading(WebView paramWebView, String paramString) {
            if ((paramWebView instanceof NWebView)) {
                ...
                if (!((NWebView) paramWebView).isBeingDragged())
    ...
    private class JSGetContentHeight {
        ...
        public void getContentHeight(WebView paramWebView, int paramInt) {
            int i = NWebView.this.getHeight();
//      if (EmbeddedWebView.this.originHeight == 0)
//        EmbeddedWebView.access$102(EmbeddedWebView.this, i);
            Log.i(NWebView.TAG, "JS getContentHeight contentHeight " + paramInt + " originHeight = " + NWebView.this.originHeight + " viewHeight = " + i);
            int j = (int) (paramInt * NWebView.this.density);
            int k = 0;
            if ((i > 0) && (j > 0)) {
                if (i - j <= 200) {
                    k = 0;
            ...
            if (k > NWebView.this.originHeight)
                k = NWebView.this.originHeight;
            if ((k > 0) && (k != i)) {
                final int m = k;
                NWebView.this.post(new Runnable() {
                    public void run() {
                        ViewGroup.LayoutParams localLayoutParams = NWebView.this.getLayoutParams();
                        localLayoutParams.height = m;
                        Log.i(NWebView.TAG, "resize height = " + localLayoutParams.height);
                        NWebView.this.setLayoutParams(localLayoutParams);
                    }
                });
            }
        }
